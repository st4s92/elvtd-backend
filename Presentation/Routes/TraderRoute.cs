...
public static class TraderRoutes
{
    ...
    {
        group.MapGet("/trader/order/{id:int}", async (int id, TraderHandler handler) =>
        {
            return await handler.GetOrder(id);
        }).WithName("GetTraderOrder").WithTags("Orders");

        group.MapGet("/trader/orders", async ([AsParameters] OrderQuery query, TraderHandler handler) =>
        {
            return await handler.GetOrders(query);
        }).WithName("GetTraderOrders").WithTags("Orders");

        group.MapGet("/trader/orders/paginated", async ([AsParameters] OrderGetPaginatedPayload query, TraderHandler handler) =>
        {
        ...
        group.MapPut("/trader/bridge/slave-order", async ([FromBody] BridgeOrderPayload payload, TraderHandler handler) =>
        {
            return await handler.HandleBridgeSlaveOrderConfirmation(payload);
        }).WithName("BridgeSlaveConfirmOrder").WithTags("Orders");

        // account
        group.MapGet("/trader/account/{id:int}", async (int id, TraderHandler handler) =>
        {
            return await handler.GetAccount(id);
        }).WithName("GetTraderAccount").WithTags("Account");

        group.MapPost("/trader/account", async ([FromBody] AccountPayload payload, TraderHandler handler) =>
        {
        ...
        group.MapGet("/trader/account/paginated", async ([AsParameters] AccountGetPaginatedPayload query, TraderHandler handler) =>
        {
        ...
        group.MapPost("/trader/master-slave", async ([FromBody] MasterSlavePayload payload, TraderHandler handler) =>
        {
            return await handler.AddMasterSlave(payload);
        }).WithName("AddTraderMasterSlave").WithTags("Master Slave");

        group.MapGet("/trader/master-slave", async ([AsParameters] MasterSlaveGetPayload query, TraderHandler handler) =>
        {
            return await handler.GetMasterSlaves(query);
        }).WithName("GetTraderMasterSlaves").WithTags("Master Slave");

        group.MapGet("/trader/master-slave/paginated", async ([AsParameters] MasterSlaveGetPaginatedPayload query, TraderHandler handler) =>
        {
            return await handler.GetPaginatedMasterSlaves(query);
        }).WithName("GetTraderPaginatedMasterSlaves").WithTags("Master Slave");

        // master slave pair
        group.MapGet("/trader/master-slave-pair/{id:int}", async (int id, TraderHandler handler) =>
        {
            return await handler.GetMasterSlavePair(id);
        }).WithName("GetTraderMasterSlavePair").WithTags("Master Slave Pair");

        group.MapPost("/trader/master-slave-pair", async ([FromBody] MasterSlavePairPayload payload, TraderHandler handler) =>
        {
            return await handler.AddMasterSlavePair(payload);
        }).WithName("AddTraderMasterSlavePair").WithTags("Master Slave Pair");

        group.MapGet("/trader/master-slave-pair", async ([AsParameters] MasterSlavePairGetPayload query, TraderHandler handler) =>
        {
            return await handler.GetMasterSlavePairs(query);
        }).WithName("GetTraderMasterSlavePairs").WithTags("Master Slave Pair");

        group.MapGet("/trader/master-slave-pair/paginated", async ([AsParameters] MasterSlavePairGetPaginatedPayload query, TraderHandler handler) =>
        {
        ...
        // master slave config
        group.MapGet("/trader/master-slave-config/{id:int}", async (int id, TraderHandler handler) =>
        {
            return await handler.GetMasterSlaveConfig(id);
        }).WithName("GetTraderMasterSlaveConfig").WithTags("Master Slave Config");

        group.MapPost("/trader/master-slave-config", async ([FromBody] MasterSlaveConfigPayload payload, TraderHandler handler) =>
        {
            return await handler.AddMasterSlaveConfig(payload);
        }).WithName("AddTraderMasterSlaveConfig").WithTags("Master Slave Config");

        group.MapGet("/trader/master-slave-config", async ([AsParameters] MasterSlaveConfigGetPayload query, TraderHandler handler) =>
        {
            return await handler.GetMasterSlaveConfigs(query);
        }).WithName("GetTraderMasterSlaveConfigs").WithTags("Master Slave Config");

        group.MapGet("/trader/master-slave-config/paginated", async ([AsParameters] MasterSlaveConfigGetPaginatedPayload query, TraderHandler handler) =>
        {
